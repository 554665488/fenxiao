...
class voucher2Model extends Model {
    ...
    public function getCurrentAvailableVoucherCount($member_id) {
        ...
        if (empty($info['voucher_count']) && $info['voucher_count'] !== 0) {
            ...
            $condition['voucher_end_date'] = array('gt',TIMESTAMP);
            $condition['voucher_state'] = 1;
            $voucher_count = $this->table('voucher2')->where($condition)->count();
    ...
    public function getVoucherList($where, $field = '*', $limit = 0, $page = 0, $order = '', $group = ''){
        ...
        if (is_array($page)){
            if ($page[1] > 0){
                $voucher_list = $this->table('voucher2')->field($field)->where($where)->limit($limit)->page($page[0],$page[1])->order($order)->group($group)->select();
            } else {
                $voucher_list = $this->table('voucher2')->field($field)->where($where)->limit($limit)->page($page[0])->order($order)->group($group)->select();
            }
        } else {
            $voucher_list = $this->table('voucher2')->field($field)->where($where)->limit($limit)->page($page)->order($order)->group($group)->select();
        }
        return $voucher_list;
    }
    ...
    public function editVoucher($data,$condition,$member_id = 0 ) {
        $result = $this->table('voucher2')->where($condition)->update($data);
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null, $order = 'voucher_id desc') {
        ...
        $field = 'voucher_id,voucher_code,voucher_price,voucher_num,voucher_state';
        //$on = 'voucher2.voucher_owner_id = member.member_id'; 
        //$where = array('voucher_owner_id'=>$member_id);
        $where['voucher_owner_id'] = $member_id;
        $where['voucher_state'] =$voucher_state;
        $list = $this->table('voucher2')->field($field)->where($where)->order($order)->page($page)->select();
     ...
     * @param int $page 分页数
     */
    public function getMemberVoucherCanList($member_id, $voucher_state, $page = null, $order = 'voucher_id desc') {
        if(empty($member_id)) {
            return false;
        }
        ...
        //$on = 'voucher2.voucher_owner_id = member.member_id'; 
        //$where = array('voucher_owner_id'=>$member_id);
        $where['voucher_owner_id']=array('neq', $member_id);
        $where['voucher_state'] =$voucher_state;
        $list = $this->table('voucher2')->field($field)->where($where)->order($order)->page($page)->select();
     ...
     *生成A金券订单
     */
    public function getVoucherOrder($template_info, $member_id, $member_name = ''){
        ...
        if($member_info['member_points']<$template_info['voucher_num']){
            return array('state'=>false,'msg'=>'增值券金额不足');
        }
        //添加A金券信息
        $insert_arr = array();
        $insert_arr['voucher_code'] = $this->get_voucher_code($member_id);
        $insert_arr['voucher_sale_time'] = time(); 
        $insert_arr['voucher_price'] = $template_info['voucher_price']; 
        $insert_arr['voucher_num']  = $template_info['voucher_num']; 
        $insert_arr['voucher_state'] = 1; 
        $insert_arr['voucher_owner_id'] = $member_id;
        $insert_arr['voucher_owner_name'] = $member_name;
        $result = $this->table('voucher2')->insert($insert_arr);
        if (!$result){
            return array('state'=>false,'msg'=>'订单生成失败');
        }
        //会员生成A金券订单日志
        $points_arr['pl_memberid'] = $member_id;
        $points_arr['pl_membername'] = $member_name;
        $points_arr['pl_points'] = -$template_info['voucher_num'];  
        $points_arr['pl_addtime'] = time();
        $points_arr['pl_desc'] = "会员卖出";
        $points_arr['pl_stage'] = "zengzhiquan";
        $result = Model('points')->savePointsLog('zengzhiquan',$points_arr,true);
        if (!$result){
            return array('state'=>false,'msg'=>'增值券卖出日志失败');
        }else{ 
     ...
     * 撤销会员A金券的数量
     */ 
    public function getVoucherOrderCancle($template_info, $member_id, $member_name = ''){
        if (intval($member_id) <= 0 || empty($template_info)){
            return array('state'=>false,'msg'=>'参数错误');
        }
        //查询会员信息
        ...
        //更新A金券信息
        $where = array(); 
        $where['voucher_id'] = $template_info['voucher_id'];   
        $update_arr = array(); 
        $update_arr['voucher_state'] = 3;   
        $update_arr['voucher_cancletime'] = time();   
        $result = $this->table('voucher2')->where($where)->update($update_arr);
        if (!$result){
            return array('state'=>false,'msg'=>'订单生成失败');
        }
        //会员撤销A金券订单日志
        ...
        $points_arr['pl_points'] = $template_info['voucher_num'];  
        $points_arr['pl_addtime'] = time();
        $points_arr['pl_desc'] = "会员卖出撤销";
        $points_arr['pl_stage'] = "zengzhiquan"; 
        $result = Model('points')->savePointsLog('zengzhiquan',$points_arr,true);
        if (!$result){
            return array('state'=>false,'msg'=>'增值券卖出撤销日志失败');
        }else{ 
            return array('state'=>true,'msg'=>'操作成功');
        }
    }
     ...
     * 会员A金券的购买
     */ 
    public function getVoucherOrderBuy($template_info, $member_id, $member_name = ''){
        if (intval($member_id) <= 0 || empty($template_info)){
            return array('state'=>false,'msg'=>'参数错误2');
        }
        $voucher_info = $this->getVoucherInfo(array('voucher_id'=>$template_info['voucher_id']));
        //return array('state'=>false,'msg'=>$voucher_info['voucher_owner_id'] );
        $predeposit=$voucher_info['voucher_num']*$voucher_info['voucher_price'];
        //查询会员信息
        if (!$member_name){
            $member_info = Model('member')->getMemberInfoByID($member_id);
            if (empty($template_info)){
                return array('state'=>false,'msg'=>'参数错误1');
            }
            $member_name = $member_info['member_name'];
        ...
        if($member_info['available_predeposit']<$predeposit){
            return array('state'=>false,'msg'=>'现金券金额不足');
        }
        //更新A金券信息
        $where = array(); 
        $where['voucher_id'] = $template_info['voucher_id'];   
        $update_arr = array(); 
        $update_arr['voucher_state'] = 0; //已卖  
        $update_arr['voucher_buy_time'] = time();   
        $update_arr['voucher_buyer_id'] = $member_id;
        $update_arr['voucher_buyer_name'] = $member_name;
        $result = $this->table('voucher2')->where($where)->update($update_arr);
        if (!$result){
            return array('state'=>false,'msg'=>'订单购买失败');
        }
        //更新现金券数量
        ...
        $data['invite_member_id']='';
         $data['member_id']= $member_id;
        $data['member_name']=$member_name;
        $data['amount']=$predeposit;
        Model('predeposit')->changePd('pay_zengzhi',$data);
        $data1 = array(); 
     
        $data1['invite_member_id']='';
         $data1['member_id']= $voucher_info['voucher_owner_id'] ;
        $data1['member_name']= $voucher_info['voucher_owner_name'] ;
        $data1['amount']=$predeposit;
        Model('predeposit')->changePd('sale_zengzhi',$data1);
        //会员购买A金券订单日志
        $points_arr = array(); 
        $points_arr['pl_memberid'] = $member_id;
        $points_arr['pl_membername'] = $member_name;
        $points_arr['pl_points'] =$voucher_info['voucher_num'];  
        $points_arr['pl_addtime'] = time();
        $points_arr['pl_desc'] = "会员购买增值券";
        $points_arr['pl_stage'] = "zengzhiquan"; 
        $result = Model('points')->savePointsLog('zengzhiquan',$points_arr,true);
        if (!$result){
            return array('state'=>false,'msg'=>'增值券购买日志失败');
        }else{ 
     ...
     * 更新会员A金券的数量
     */
    private function _updateMemberPoints($member_id,$voucher_num) {
        $condition = array();
        $condition['member_id'] = $member_id; 
        $this->table('member')->where($condition)->update(array('member_points' => $voucher_num));
    }
    /**
     * 查询代金券模板列表
     */
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = ''){
        ...
        if ($template_info['voucher_t_points'] > 0 && $template_info['voucher_t_gettype'] == $this->voucher_gettype_array['points']['sign']){
            ...
            $points_arr['point_ordersn'] = $insert_arr['voucher_code'];$result = Model('points')->savePointsLog('app',$points_arr,true);
    ...
    public function addVoucherBatch($insert_arr){
        return $this->table('voucher2')->insertAll($insert_arr);
    }
    /**
     * 获得代金券模板总数量
     */
    ...
    public function getVoucherCount($where){
        return $this->table('voucher2')->where($where)->count();
    }
    /**
     * 获得代金券模板详情
     */
    ...
    public function getVoucherInfo($where = array(), $field = '*', $order = '',$group = '') {
        $info = $this->table('voucher2')->where($where)->field($field)->order($order)->group($group)->find();
     ...
     *增值券提币
     */
    public function addtibi($card){
        
        try { 
            $this->table('ajinquan')->insert($card);
               //更新member内容
            $obj_member = Model('member');
            $upmember_array = array(); 
            $upmember_array['member_points'] = array('exp','member_points-'.$card['a_amount']);
            $upmember_array['points_cashout'] = array('exp','points_cashout+'.$card['a_amount']);
            $obj_member->editMember(array('member_id'=>$card['a_member_id']),$upmember_array);
             //会员生成A金券订单日志
             $points_arr = array();
            $points_arr['pl_memberid'] =$card['a_member_id'] ;
            $points_arr['pl_membername'] =$card['a_member_name'] ;
            $points_arr['pl_points'] =$card['a_amount'];  
            $points_arr['pl_addtime'] = time();
            $points_arr['pl_desc'] = "增值券提币";
            $points_arr['pl_stage'] = "tibi";
            $result = Model('points')->addPointsLog($points_arr);
     ...
     *增值券提币拒绝
     */
    public function refusetibi($where,$card){
        
        try { 
            //更新
           // $this->table('ajinquan')->insert($card);
            $this->table('ajinquan')->where($where)->update($card);
               //更新member内容
            $obj_member = Model('member');
            $upmember_array = array(); 
            $upmember_array['member_points'] = array('exp','member_points+'.$card['a_num']);
            $upmember_array['points_cashout'] = array('exp','points_cashout-'.$card['a_num']);
            $obj_member->editMember(array('member_id'=>$card['a_owner_id']),$upmember_array);
             //会员生成A金券订单日志
             $points_arr = array();
            $points_arr['pl_memberid'] =$card['a_owner_id'] ;
            $points_arr['pl_membername'] =$card['a_owner_name'] ;
            $points_arr['pl_points'] =$card['a_num'];  
            $points_arr['pl_addtime'] = time();
            $points_arr['pl_desc'] = "增值券拒绝提币";
            $points_arr['pl_stage'] = "tibi";
            $result = Model('points')->addPointsLog($points_arr);
     ...
     *增值券提币同意
     */
    public function aggrentibi($where,$card){
     ...
     *兑换仓转换
     */
    public function changeDuiHuan($card){
        ...
        try { 
            ...
            $points_arr['pl_points'] =$card['a_amount'];  
            $points_arr['pl_addtime'] = time();
            $points_arr['pl_desc'] = $card['pl_desc'];  
            $points_arr['pl_stage'] = $card['pl_stage'];  
            $result = Model('points')->addPointsLog($points_arr);
