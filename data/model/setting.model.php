...
/**
    ...
    public function getRewardSetting(){
        $param = array();
        $param['table'] = 'setting';
        $param['where'] = "name='baodan_config'";
        $result = Db::select($param);
        /**
         * 整理
         */
     ...
     * @return bool 布尔类型的返回结果
     */
    public function updateRewardSetting($param){
        if (empty($param)){
            return false;
        }
        $config = $this->getRewardSetting();
        $row = array('value' => serialize($param));
        if(empty($config)){
            $row['name'] = "baodan_config";
            $result = Db::insert('setting',$row);
                if ($result !== true){
                    return $result;
                }
        }  else {
                $where = " name = 'baodan_config'";
                $result = Db::update('setting',$row,$where);
                if ($result !== true){
                    return $result;
                }
        }
