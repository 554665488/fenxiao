...
class orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        ...
        if (isset($order_info['order_state'])) {
            ...
            if(isset($order_info['is_wholesale']) && $order_info['is_wholesale'] == 1) {
              $state = '';
              if($order_info['sale_state'] == 0) {
                $state = '待预售';
              } else if($order_info['sale_state'] == 1) {
                $state = '预售中';
              }  else if($order_info['sale_state'] == 20) {
                $state = '预售完成';
              }
              if($order_info['order_state'] == ORDER_STATE_PAY) {
                $order_info['state_desc'] = $state;
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false){
        ...
        foreach ($list as $order) {
            if (isset($order['order_state'])) {
                ...
                if(isset($order['is_wholesale']) && $order['is_wholesale'] == 1) {
                  $state = '';
                  if($order['sale_state'] == 0) {
                    $state = '待预售';
                  } else if($order['sale_state'] == 1) {
                    $state = '预售中';
                  }  else if($order['sale_state'] == 20) {
                    $state = '预售完成';
                  }
     ...
     * @param unknown $condition
     */
    public function getOrderStateWaitSale($condition = array()) {
     ...
     * @param unknown $condition
     */
    public function getOrderStateInSale($condition = array()) {
        $condition['order_state'] = array('in',array(ORDER_STATE_PAY));
        $condition['is_wholesale'] = array('gt',0);
    ...
    public function getOrderAmount($condition = array()) {
        return $this->table('orders')->where($condition)->sum('order_amount');
    }

    public function getOrderAndOrderGoodsAmount($condition) {
        return $this->table('order_goods,orders')->join('inner')->on('order_goods.order_id=orders.order_id')->where($condition)->sum('order_goods.goods_pay_price');
    }
}
