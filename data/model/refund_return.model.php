...
class refund_returnModel extends Model{
    ...
    public function editOrderRefund($refund,$user = '') {
        ...
        if ($refund_id > 0) {
            ...
            try {
                ...
                $predeposit_amount = $order_amount-$order['refund_amount']-$rcb_amount;//可退现金券金额
                $detail_array = array();
                ...
                if ($predeposit_amount > 0) {//退现金券
                    $log_array = array();
                    $log_array['member_id'] = $order['buyer_id'];
                    $log_array['member_name'] = $order['buyer_name'];
                    $log_array['order_sn'] = $order['order_sn'];
                    $log_array['amount'] = $refund['refund_amount'];//退现金券金额
                    ...
                    if ($log_array['amount'] > 0) {
                        ...
                        $state = $model_predeposit->changePd('refund', $log_array);//增加买家可用现金券金额
                    }
                }
    ...
    public function getPayDetailInfo($detail_array) {
        ...
        $order['pay_amount'] = ncPriceFormat($order['order_amount']-$order['rcb_amount']-$order['pd_amount']);//在线支付金额=订单总价格-充值卡支付金额-现金券支付金额
