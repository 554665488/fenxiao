...
class pointsModel extends Model {
    ...
    public function __construct(){
        parent::__construct();
        $this->stage_arr = array('regist'=>'注册','login'=>'登录','comments'=>'商品评论','order'=>'订单消费',
        'system'=>'积分管理','pointorder'=>'积分兑换','app'=>'积分兑换','signin'=>'签到','inviter'=>'邀请返利',
        'rebate'=>'消费返利','pointstomoney' => '积分抵扣','pointsdis' => '积分折扣','freeze_user_buy' => "冻结仓返利",
        'freeze_to_release' => "冻结仓释放", 'sys_coupon' => '兑换仓管理', 'sys_og' => '原始仓管理',
         'sys_freeze' => '冻结仓管理', 'transfer_to' => '转赠', 'transfer_recv' => '接收','order_return' => '购买赠送'
      );
    }
    /**
     * 操作积分
    ...
    function savePointsLog($stage,$insertarr,$if_repeat = true){
        ...
        switch ($stage){
            ...
            case 'order':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '订单'.$insertarr['order_sn'].'购物消费返还';
                }
                $insertarr['pl_points'] = 0;
                if($insertarr['points'] > 0) {
                  $insertarr['pl_points'] = $insertarr['points'];
                }
                else if ($insertarr['orderprice']){
            ...
            case 'pointorder':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '兑换礼品信息'.$insertarr['point_ordersn'].'消耗A金券';
                }
                break;
            case 'app':
                if (!$insertarr['pl_desc']){
        ...
        if ($result){
            ...
            if($stage == "pointstomoney") {
              $upmember_array['coupon_point'] = array('exp','coupon_point+'.$insertarr['pl_points']);
            } else {
     ...
     * @return bool
     */
    function savePointsOtherLog($stage,$insertarr,$ys = true){
        if (!$insertarr['pl_memberid']){
            return false;
        }
        //记录原因文字
        ...
        if ($result){
            ...
            if($ys) {
              ...
              $upmember_array['points_original'] = array('exp','points_original+'.$insertarr['pl_points']);
			  if($stage == 'sys_og') {

			  }
            } else {
              $upmember_array['points_freeze'] = array('exp','points_freeze+'.$insertarr['pl_points']);
            }
            if($stage == "freeze_to_release") {
              $upmember_array['member_points'] = array('exp','member_points+'.abs($insertarr['pl_points']));
            }
            $obj_member->editMember(array('member_id'=>$insertarr['pl_memberid']),$upmember_array);
            return true;
        }else {
...
*操作兑换
*/
    function savePointsCouponLog($stage,$insertarr){
        if (!$insertarr['pl_memberid']){
            return false;
        }
        //记录原因文字
    ...
    public function getLastPrice() {
      ...
      return floatval($list_setting['Aquan']);
    }
}
