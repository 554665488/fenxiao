...
class buy_1Logic {
    ...
    /**
     * 现金券支付,依次循环每个订单
     * 如果现金券足够就单独支付了该订单，如果不足就暂时冻结，等API支付成功了再彻底扣除
     */
    ...
    /*
       ...
       * 抵用后直接扣除用户积分
       */
       public function points2Play($order_list,  $member_id){
         ...
         try {
           ...
           foreach ($order_list as $order_info) {
             ...
             if($need_points2 > 0) {
               if(floatval($od['points_number2']) > 0) {
                 continue;
               }
               $data_order['points_number2'] = $need_points2;
             }
             if($need_points > 0) {
               if(floatval($od['points_number']) > 0) {
                 continue;
               }
               $data_order['points_number'] = $need_points;
             }
             //更新订单
             $result = $model_order->editOrder($data_order,array('order_id'=>$order_info['order_id']));
             ...
             if($np2 > 0) {
               ...
               $insert_arr['pl_memberid'] = $member_id;
               $insert_arr['pl_membername'] = $member_name;
               $insert_arr['pl_points'] = -$data_order['points_number2'];
               $insert_arr['order_sn'] = $order_info['order_sn'];
               $model_points2->savePointsLog('pointstomoney',$insert_arr,true);
             }
             if($np > 0) {
               //扣除会员积分//写订单日记
               $insert_arr = array();
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        // 商品信息
        $new_array['goods_apoint_rate'] = $goods_info['goods_apoint_rate'];
        $new_array['goods_point_rate2'] = $goods_info['goods_point_rate2'];
        $new_array['z_goods_type'] = $goods_info['z_goods_type'];
        $new_array['points2'] = $goods_info['points2'];
