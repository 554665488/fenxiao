...
class buyLogic {
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id, $jjg = null,$orderdiscount = 0,$member_level = 0,$ifchain = null) {
        ...
        //得到页面所需数据：收货地址、发票、代金券、现金券、商品列表等信息
		...
		foreach ($goods_list as $goods) {
          if($goods['z_goods_type'] == 1) {
            ...
            if(count($goods_list) > 1) {
              throw new Exception('折扣区产品一次只能购买一种');
            }
		  } else if($goods['z_goods_type'] == 2) {
            $this->_order_data['z_goods_type'] = 2;
            if(count($goods_list) > 1) {
              throw new Exception('平价区产品一次只能购买一种');
            }
		  } else {
			  if(count($goods_list) > 1) {
				  throw new Exception('零售区产品一次只能购买一种');
			  }
			  if(intval($goods_list[0]['goods_num']) > 1) {
				  throw new Exception('零售区产品一次只能购买一个');
			  }
			...
			$stime = $list_setting['starttime'];
			if(empty($stime)) {
				$stime = '09:00:00';
			}
			$etime = $list_setting['endtime'];
			if(empty($stime)) {
				$etime = '16:00:00';
			}
			$d1 = date('Y-m-d') . ' '. $stime;
...
//             }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) { //bl_id = 0
                    ...
                    if($z_goods_type == 1 || $z_goods_type == 2) {
                      ...
                      $po = floatval($goods_info['points2']) * floatval($goods_info['goods_num']);
                      if($z_goods_type == 1) {
                        $totalpoints2 += $po;
                      }
                      else if($z_goods_type == 2){
                        $totalpoints += $po;
                      }
                    }
    ...
    private function _createOrderStep5() {
		...
		//$order_list = $this->_logic_buy_1->pointsPlay($this->_order_data['order_list'], $this->_post_data['J_PointInput'], $this->_member_info['member_id']);
