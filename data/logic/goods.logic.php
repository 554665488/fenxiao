...
class goodsLogic {
    ...
    public function saveGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc) {
		...
		if($param['g_goods_type'] == 1 && empty($param['g_goods_common_id'])) {
			return callback(false, '折扣商品必须添加对应商品SPU');
		}
        // 验证参数
    ...
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state) {
        ...
        $common_array['goods_apoint_rate'] = floatval($param['g_apointreturn']);
        $common_array['goods_point_rate2'] = floatval($param['g_pointreturn2']);
        $common_array['z_goods_type'] = floatval($param['g_goods_type']);
        $common_array['sort'] = intval($param['g_sort']);
        $common_array['points2'] = floatval($param['g_points2']);
		$common_array['sale_goods_common_id'] = intval($param['g_goods_common_id']);
        return $common_array;
    }
    /**
     * 序列化保存手机端商品描述数据
    ...
    private function _initGoodsByCommonGoods($common_id, $common_array) {
        ...
        $goods['goods_apoint_rate'] = $common_array['goods_apoint_rate'];
        $goods['goods_point_rate2'] = $common_array['goods_point_rate2'];
        $goods['z_goods_type'] = $common_array['z_goods_type'];
        $goods['sort'] = $common_array['sort'];
        $goods['points2'] = $common_array['points2'];
		$goods['sale_goods_common_id'] = $common_array['sale_goods_common_id'];
        return $goods;
    }
