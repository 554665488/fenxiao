    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '', $if_update_account = true, $cancel_condition = array()) {
        try {
            ...
            if ($if_update_account) {
				...
				//还原积分
				$pointnumber = floatval($order_info['points_number2']);
				if($pointnumber > 0) {
					Model('points2')->savePointsLog('pointstomoney',
						array('pl_memberid'=>$order_info['buyer_id'],
						  'pl_membername'=>$order_info['buyer_name'],
						  'orderprice'=>$order_info['order_amount'],
						  'order_sn'=>$order_info['order_sn'],
						  'pl_points' => $order_info['points_number'],
						  'pl_desc'=> '订单取消优惠券返还,订单号:'.$order_info['order_sn']),
						true);
				}
            }
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array()) {
        ...
        try {
            ...
            foreach($order_list as $order_info) {
                ...
                $aprice = Model('points')->getLastPrice();
                $update_order['apoints_get_price'] = $aprice;
                if ($post['trade_no'] != '') {
                ...
                //Model('predeposit')->setOrderApointsPrice($order_id);
                if (!$update) {
                    throw new Exception('操作失败');
                }
            }
