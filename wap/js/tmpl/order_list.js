...
$(function(){
	...
	$('#order-list').on('click','.sale-order',function() {
		...
		$.sDialog({
			content: msg,
			okFn: function() { goSale(order_id, type); }
		});
	});
	...
	function goSale(order_id, type) {
		$.ajax({
			type: 'post',
			url: ApiUrl + "/index.php?act=member_order&op=go_sale",
			data:{key:key,order_id:order_id,type:type},
			dataType:'json',
			success:function(result) {
