...
class member_accountControl extends mobileMemberControl
{
     ...
     * 更改密码 原密码更改
     */
    public function modify_password_witholdOp()
    {
        //先获取短信验证码
        $redisConfig = C('redis');
        $redis = new Redis();
        ...
        if (empty($_POST['code'])) {
            ...
            $code = $_POST['code'];
            if (!$redis->exists('modify_password_code_' . $_POST['member_mobile']) or $redis->get('modify_password_code_' . $_POST['member_mobile']) != $code) {
                output_error('验证码错误');
            }
        }
        ...
        if ($this->member_info['member_passwd'] != md5($_POST['passwordold'])) {
            output_error('原密码不正确');
        }
        $model_member = Model('member');
    ...
    public function modify_paypassword_witholdOp()
    {
        ...
        if ($this->member_info['member_paypwd'] != md5($_POST['passwordold'])) {
            output_error('原密码不正确');
        }
        $model_member = Model('member');
    ...
    public function pd_cash_rateOp()
    {
        $model_setting = Model('setting');
        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID($this->member_info['member_id']);
        $list_setting = $model_setting->getRewardSetting();
        $rate = $list_setting['cashout_fee'];

        output_data(array('rate' => $rate));
    }
}
