    ...
    public function indexOp(){
        ...
        if(!empty($_GET['code'])) {
            ...
            if(!empty($user_info['unionid'])){
                ...
                $state_data = array();
                $token = 0;
                if(!empty($member) && !empty($member_mobile)) {//会员信息存在时自动登录
                    ...
                    redirect(WAP_SITE_URL.'/tmpl/member/bandingPhone.html?types=wap&recmember='.$recmember.'&unionid=&userinfo='.urlencode(json_encode($user_info)));
		        ...
		        $_url = $logic_connect_api->getWxOAuth2Url($recmember);
        ...
        //获取到￥recmember 
            ...
            if(!empty($unionid)) { 
                ...
                if(!empty($member)) {//会员信息存在时自动登录
                    ...
                    redirect(WAP_SITE_URL.'/tmpl/member/bandingPhone.html?types=app&recmember=&unionid='.$unionid.'&userinfo=');
