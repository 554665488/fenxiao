...
class points2Control extends SystemControl{
    const EXPORT_SIZE = 5000;
    public function __construct(){
        parent::__construct();
        Language::read('points2');
        //判断系统是否开启积分功能
        if (C('points_isuse') != 1){
    ...
    public function addpointsOp(){
        if (chksubmit()){
            ...
            if (!is_array($member_info) || count($member_info)<=0){
                showMessage(Language::get('admin_points_userrecord_error'),'index.php?act=points2&op=addpoints','','error');
            }

            $pointsnum = intval($_POST['pointsnum']);
            if ($_POST['operatetype'] == 2 && $pointsnum > intval($member_info['member_points2'])){
                showMessage(Language::get('admin_points_points_short_error').$member_info['member_points2'],'index.php?act=points2&op=addpoints','','error');
            }
            ...
            if ($result){
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?act=points2&op=addpoints');
            }else {
                showMessage(Language::get('nc_common_save_fail'),'index.php?act=points2&op=addpoints','','error');
            }
        }else {
			Tpl::setDirquna('shop');
            Tpl::showpage('points.add2');
        }
    }
    public function checkmemberOp(){
        ...
        $obj_member = Model('member');
        $member_info = $obj_member->getMemberInfo(array('member_mobile'=>$name));
        if (is_array($member_info) && count($member_info)>0){
            if(strtoupper(CHARSET) == 'GBK'){
                $member_info['member_name'] = Language::getUTF8($member_info['member_name']);
            }
            echo json_encode(array('id'=>$member_info['member_id'],'name'=>$member_info['member_name'],'points'=>$member_info['member_points2']));
        }else {
            echo ''; die;
        }
    }
