...
class memberControl extends SystemControl{
    ...
    public function get_xmlOp() {
        ...
        foreach ($member_list as $value) {
            ...
            $c = $model_member->getMemberInfoByID($value['inviter_id'],'member_name,member_id,member_mobile,member_truename');
            $param['member_inviter'] = $c['member_name']."(".$c['member_truename'].")";
            ...
            $param['member_points2'] = $value['member_points2'];
    ...
    private function createCsv($member_list) {
        ...
        foreach ($member_list as $value) {
            ...
            $c = $model_member->getMemberInfoByID($value['inviter_id'],'member_name,member_id');
            $param['member_inviter'] = $c['member_name'];
            $param['achievement'] = $value['achievement'];
            $param['member_mobile'] = $value['member_mobile'];
            $param['level_name'] = "普通会员";
            if($value['level'] == 1) {
              $param['level_name'] = "一级会员";
            } else if($value['level'] == 2) {
              $param['level_name'] = "二级会员";
            } else if($value['level'] == 3) {
              $param['level_name'] = "三级会员";
            } else if($value['level'] == 4) {
              $param['level_name'] = "一级消费商";
            } else if($value['level'] == 5) {
              $param['level_name'] = "二级消费商";
            } else if($value['level'] == 6) {
              $param['level_name'] = "三级消费商";
            } else if($value['level'] == 7) {
              $param['level_name'] = "县级代理";
            } else if($value['level'] == 8) {
              $param['level_name'] = "市级代理";
            } else if($value['level'] == 9) {
              $param['level_name'] = "省级代理";
            }
            $param['member_truename'] = $value['member_truename'];
            $param['member_time'] = date('Y-m-d', $value['member_time']);
            $param['member_login_time'] = date('Y-m-d', $value['member_login_time']);
            $param['member_login_ip'] = $value['member_login_ip'];
            $param['points_original'] = $value['points_original'];
            $param['points_freeze'] = $value['points_freeze'];
            $param['member_points'] = $value['member_points'];
            $param['available_predeposit'] = ncPriceFormat($value['available_predeposit']);
            $param['freeze_predeposit'] = ncPriceFormat($value['freeze_predeposit']);
