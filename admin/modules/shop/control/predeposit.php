...
class predepositControl extends SystemControl{
	...
	if(!empty($_GET['state'])) {
	   if($_GET['state'] == 'payed') {
		Tpl::showpage('pd_cash.list.payed');
	   } else if($_GET['state'] == 'notpayed') {
		Tpl::showpage('pd_cash.list.rej');
	   }
	   return;
	}
    ...
    public function pd_cash_delOp(){
        ...
        if ($id > 0) {
            ...
            if (!$info) {
                exit(json_encode(array('state'=>false,'msg'=>'操作失败，信息不存在')));
            }
            try {
                $result = $model_pd->editPdCash(array('pdc_payment_state' => 2),$condition);
                if (!$result) {
	...
	$rate = floatval($list_setting['cashout_fee']);
	$info['fee'] = round($info['pdc_amount'] * $rate,2);
	$info['real_amount'] = $info['pdc_amount'] - $info['fee'];
	$info['fee_rate'] = $rate;
        Tpl::output('info',$info);
		Tpl::setDirquna('shop');
    ...
    public function get_cash_xmlOp() {
        ...
        $data['now_page'] = $model_pd->shownowpage();
        $data['total_num'] = $model_pd->gettotalnum();
	$payment_state = array(0=>'未支付', 1=>'已支付', 2=>'已拒绝');
        foreach ($cash_list as $value) {
            $param = array();
            $param['operation'] = "";
            if ($value['pdc_payment_state'] == 0) {
                $param['operation'] .= "<a class='btn red' href=\"javascript:void(0)\" onclick=\"fg_delete('" . $value['pdc_id'] . "')\"><i class='fa fa-trash-o'></i>拒绝</a>";
            }
            ...
            $param['pdc_payment_state'] = $payment_state[$value['pdc_payment_state']];
            $param['pdc_payment_time'] = $value['pdc_payment_time'] > 0 ? date('Y-m-d H:i:s', $value['pdc_payment_time']) : '';
    ...
    //        $param['operation'] = "--";
	    ...
	    $param['lg_av_before'] = ncPriceFormat($value['lg_av_before']);
            $param['lg_freeze_amount'] = ncPriceFormat($value['lg_freeze_amount']);
            $param['lg_add_time'] = date('Y-m-d H:i:s', $value['lg_add_time']);
