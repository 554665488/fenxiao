...
class saleorderControl extends SystemControl{
    /**
     * 每次导出订单数量
     * @var int
     */
    ...
    public function indexOp(){
        ...
        Tpl::showpage('saleorder.index');
    }

    public function saledOp(){
        //显示支付接口列表(搜索)
        $payment_list = Model('payment')->getPaymentOpenList();
        $payment_list['wxpay'] = array(
            'payment_code' => 'wxpay',
	...
	$condition['order_state'] = array('egt',20);
        $condition['sale_state'] = 1;//挂卖中
	$this->_get_condition($condition);
	
	if(!empty($_GET['saled'])) {
	    $condition['sale_state'] = 20;
	}

        $sort_fields = array('buyer_name','store_name','order_id','payment_code','order_state','order_amount','order_from','pay_sn','rcb_amount','pd_amount','payment_time','finnshed_time','evaluation_state','refund_amount','buyer_id','store_id','sale_date');
	...
	$orderdesc = array(0=>'待交割',1=> '交割中',20=>'已完成');
        foreach ($order_list as $order_id => $order_info) {
            ...
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?act=saleorder&op=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            if ($order_info['if_system_receive_pay']) {
                ...
                $operation_detail .= "<li><a href=\"index.php?act=saleorder&op=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">{$op_name}</a></li>";
            }
            if ($operation_detail) {
            ...
            $list['sale_date'] = date('Y-m-d H:i:s',$order_info['sale_date']);
			...
			$list['order_state'] = $orderdesc[$order_info['sale_state']];
            ...
            $list['sale_amount'] = ncPriceFormat($order_info['sale_amount']);
	    $list['sale_fee'] = ncPriceFormat($order_info['sale_fee']);
			$list['finnshed_time'] = !empty($order_info['sale_finish_date']) ? date('Y-m-d H:i:s',$order_info['sale_finish_date']) : '';
    ...
    public function sellte_orderOp() {
      $orderids  = array();
      if (preg_match('/^[\d,]+$/', $_GET['order_id'])) {
          $orderids = explode(',',trim($_GET['order_id'],','));
      }
      if(empty($orderids)) {
        showMessage('参数为空','','html','error');
      }
      if(count($orderids) > 20) {
        showMessage('选择是项目太多','','html','error');
      }
      $pd = Model('predeposit');
      if($pd->settleSaleOrder($orderids)) {
        showMessage('结算成功','','html');
      } else {
        showMessage('结算失败','','html','error');
      }
    }
    /**
     * 导出
    ...
    private function createExcel($data = array()){
        ...
        foreach ((array)$data as $k=>$order_info){
            ...
            $list['order_sn'] = $order_info['order_sn'].str_replace(array(1,2,3), array(null,' [预定]','[门店自提]'), $order_info['order_type']);
            $list['order_from'] = str_replace(array(1,2), array('PC端','移动端'),$order_info['order_from']);
            $list['add_time'] = date('Y-m-d H:i:s',$order_info['sale_date']);
            $list['order_amount'] = ncPriceFormat($order_info['order_amount']);
            if ($order_info['shipping_fee']) {
                $list['order_amount'] .= '(含运费'.ncPriceFormat($order_info['shipping_fee']).')';
            }
	...
	if($_GET['buyer_name'] != '') {
        ...
        if (!in_array($_GET['qtype_time'],array('add_time','payment_time','sale_date'))) {
        ...
        if(in_array($_GET['order_state'],array('0','1','20','30','40'))){
            $condition['sale_state'] = $_GET['order_state'];
        }
