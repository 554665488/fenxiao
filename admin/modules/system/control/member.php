...
class memberControl extends SystemControl{
	...
	private $links = array(
		...
		array('url'=>'act=member&op=treeview','lang'=>'member_index_treeview'), 
    );
    ...
    public function member_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
              ...
              $condition['member_id'] = intval($_POST['member_id']);
              $member_array = $model_member->getMemberInfo($condition);
                ...
                $update_array['achievement']         = $_POST['achievement'];
                $update_array['total_achievement']          = $_POST['total_achievement'];
				...
				$update_array['level']       = intval($_POST['level']);
        $update_array['level2']       = intval($_POST['level2']);
        $update_array['level3']       = intval($_POST['level3']);
				$update_array['vip']       = intval($_POST['vip']);
        if($update_array['vip'] != $member_array['vip']) {
          $update_array['vip_date'] = date('Y-m-d H:i:s');
        }
				...
				if(!empty($_POST['province'])) {
                    $update_array['member_provinceid'] = $_POST['province'];
					$area = $model_area->getAreaInfo(array('area_id' => $update_array['member_provinceid']));
					$update_array['member_areainfo'] = $area['area_name'];
                }else {
                    $update_array['member_provinceid'] = NULL;
                }
                if(!empty($_POST['city'])) {
                    $update_array['member_cityid'] = $_POST['city'];
					$area = $model_area->getAreaInfo(array('area_id' => $update_array['member_cityid']));
					$update_array['member_areainfo'] .= ' '.$area['area_name'];
                }else {
                    $update_array['member_cityid'] = NULL;
                }
                if(!empty($_POST['area'])) {
                    $update_array['member_areaid'] = $_POST['area'];

					/*$area = $model_area->getAreaInfo(array('area_id' => $update_array['member_areaid']));
					$city = $model_area->getAreaInfo(array('area_id' => $area['area_parent_id']));
					$province = $model_area->getAreaInfo(array('area_id' => $city['area_parent_id']));
					$update_array['member_areainfo'] = $province['area_name'].' '.$city['area_name'].' '.$area['area_name'];*/
					$area = $model_area->getAreaInfo(array('area_id' => $update_array['member_areaid']));
					$update_array['member_areainfo'] .= ' '.$area['area_name'];
                }else {
                    $update_array['member_areaid'] = NULL;
                }
	...
	public function treeviewOp(){
            $obj_member = Model('member');
            if(!empty($_GET["node"])){
                $list = $obj_member->getMemberList(array('invite_one'=>$_GET["node"]),"member_id,member_name,achievement");
                $rs = array();
                foreach ($list as $l) {
                    $rs[] = array("label"=>$l['member_name'],"id"=>$l['member_id'],'load_on_demand'=>true, 'ac' => $l['achievement'] > 0);
                }
                echo json_encode($rs);
                return;
            }
            Tpl::output('top_link',$this->sublink($this->links,'treeview'));
            Tpl::setDirquna('system');
            $mid = intval($_GET['mid']);

            $list = $obj_member->getMemberList(array('invite_one'=>$mid),"member_id,member_name,achievement");
            $rs = array();
            foreach ($list as $l) {
                $rs[] = array("label"=>$l['member_name'],"id"=>$l['member_id'],'load_on_demand'=>true, 'ac' => $l['achievement'] > 0 );
            }
            Tpl::output("data",  json_encode($rs));
            Tpl::showpage('member.treeview');
        }
}
