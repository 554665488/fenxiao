...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      </div><ul class="tab-base nc-row"><li><a  href="index.php?act=saleorder"><span>出售中</span></a></li><li><a class="current" ><span>已完成</span></a></li></ul> 
    </div>
  </div>
  <!-- 操作说明 -->
  ...
  <div class="ncap-search-bar">
    ...
    <form method="get" name="formSearch" id="formSearch">
            ...
            <dd>
              <label>
                <select class="s-select" name="qtype_time">
                  ...
                  <option value="sale_date">挂卖时间 </option>
                </select>
              </label>
              <label>
	  ...
	  <dl>
            ...
            <dd>
              <label>
                <input placeholder="请选输入买家账号" name="buyer_name" value="" type="text" class="s-input-txt" />
              </label>
            </dd>
          </dl>
          <dl>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=saleorder&op=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=saleorder&op=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?act=saleorder&op=get_xml&saled=true',
        colModel : [
			...
			{display: '挂卖时间', name : 'sale_date', width : 140, sortable : true, align: 'left'},
			{display: '订单金额', name : 'order_amount', width : 140, sortable : true, align: 'left'},
            ...
            {display: '预存款支付', name : 'pd_amount', width : 100, sortable : false, align: 'center'},
	    {display: '结算收入', name : 'sale_amount', width : 80, sortable : true, align: 'center'},
	    {display: '结算手续费', name : 'sale_fee', width : 80, sortable : true, align: 'center'},
        ...
        buttons : [
            ...
            {display: '<i class="fa fa-print"></i>结算', name : 'saleok', bclass : 'saleok', title : '结算订单，相当于用户挂卖成功', onpress : fg_operate }
        ],
        searchitems : [
...
function fg_operate(name, grid) {
    if (name == 'csv') {
      ...
      var itemlist = new Array();
        ...
        fg_settleOrder(itemlist);
    }
}
var has_settle = false;
function fg_settleOrder(ids) {
  if(!has_settle && confirm('是否结算所选择项？')) {
    has_settle = true;
    id = ids.join(',');
    window.location.href = 'index.php?act=saleorder&op=sellte_order&order_id=' + id;
  }
}
...
function fg_cancel(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?act=saleorder&op=change_state&state_type=cancel",
        data: "order_id="+id,
