...
<div class="page">
  ...
  <form id="user_form" enctype="multipart/form-data" method="post">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        <dt class="tit">
          <label for="achievement">总消费额</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $output['member_array']['achievement'];?>" id="achievement" name="achievement" class="input-txt">
          <span class="err"></span>
          <p class="notic">个人消费总额</p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
          <label for="total_achievement">业绩</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $output['member_array']['total_achievement'];?>" id="total_achievement" name="total_achievement" class="input-txt">
          <span class="err"></span>
          <p class="notic">团队总业绩</p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
...
function load_area(selector,parent_area_id,select_area_id) {
     $.getJSON('<?php echo ADMIN_SITE_URL?>/index.php?act=common&op=get_area&parent=' + parent_area_id,function(data) {
        ...
        $(selector).html(html);
        if(select_area_id) {
            $(selector).val(select_area_id);
        }
    });
}
$(function(){
    load_area("#province",0,'<?php echo $output['member_array']['member_provinceid']; ?>');
    <?php if($output['member_array']['member_provinceid']) { ?>
        load_area("#city",<?php echo $output['member_array']['member_provinceid']; ?>,'<?php echo $output['member_array']['member_cityid']; ?>');
    <?php }?>
    <?php if($output['member_array']['member_cityid']) { ?>
        load_area("#area",<?php echo $output['member_array']['member_cityid']; ?>,'<?php echo $output['member_array']['member_areaid']; ?>');
    <?php }?>
    $("#province").on('change', function() {
        load_area("#city",$(this).val());
    });
    $("#city").on('change', function() {
        load_area("#area",$(this).val());
    });
});
</script>
