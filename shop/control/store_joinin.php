...
class store_joininControl extends BaseHomeControl {
    ...
    private function step3_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['bank_account_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"40","message"=>"银行账号不能为空且必须小于20个字"),
    ...
    public function check_seller_name_existOp() {
        ...
        if (!$result) {
            //查看用户的申请状态 申请状态 10-已提交申请 11-缴费完成  20-审核成功 30-审核失败 31-缴费审核失败 40-审核通过开店
            $condition['joinin_state'] = array('not in','30,31');
	...
	private function getStoreInfo(){
        $model_store_joinin = Model('store_joinin');
        $rs = $model_store_joinin->getOne(array('member_id'=>$_SESSION['member_id']));
        //var_dump($rs);
//        var_dump($_SESSION);
