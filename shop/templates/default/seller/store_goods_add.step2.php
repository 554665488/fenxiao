...
<div class="item-publish">
  <form method="post" enctype="multipart/form-data" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        <dt>排序<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_sort" value="<?php echo $output['goods']['sort']; ?>" type="text" class="text w60" /> <span></span>
          <p class="hint">排序为整数，数值越大，越靠前排。</p>
        </dd>
      </dl>
      <dl>
        <dt>优惠券返还数量<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_pointreturn2" value="<?php echo $output['goods']['goods_point_rate2']; ?>" type="text" class="text w60"  />
          <p class="hint">优惠券返还数量。</p>
        </dd>
      </dl>
      <dl>
        <dt>类型<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <ul class="ncsc-form-radio-list">
            <li>
              <label>
                <input name="g_goods_type" value="0" <?php if (!empty($output['goods']) && $output['goods']['z_goods_type'] == 0) { ?>checked="checked" <?php } ?> type="radio" />
                零售区</label>
            </li>
            <li>
              <label>
                <input name="g_goods_type" value="1" <?php if (empty($output['goods']) || $output['goods']['z_goods_type'] == 1) { ?>checked="checked" <?php } ?> type="radio"/>
                折扣区</label>
            </li>
            <li>
              <label>
                <input name="g_goods_type" value="2" <?php if (empty($output['goods']) || $output['goods']['z_goods_type'] == 2) { ?>checked="checked" <?php } ?> type="radio"/>
                平价区</label>
            </li>
          </ul>
          <p class="hint"></p>
      ...
      <dl>
        <dt>优惠券或积分数量<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_points2" value="<?php echo $output['goods']['points2']; ?>" type="text" class="text w60"  />
          <p class="hint">1.若为折扣区产品，为抵扣优惠券数量，2.若为平价区产品，为抵扣积分数量</p>
        </dd>
      </dl>
	  <dl>
        <dt>折扣区对应零售区商品SPU<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_goods_common_id" value="<?php echo $output['goods']['sale_goods_common_id']; ?>" type="text" class="text w60"  />
          <p class="hint">1.若为折扣区产品，需要填写商品的SPU</p>
        </dd>
      </dl>
